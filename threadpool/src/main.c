...
int main(int argc, char** argv) {
    ...
    while ((ch = getopt(argc, argv, "t:k:")) != -1) {
        switch (ch) {
            case 't': taskcnt = atoi(optarg); break;
            case 'k': threadcnt = atoi(optarg); break;
            case '?':
            default: printf("Usage: %s [-k threadcnt] [-t taskcnt]\n" "\tdefault threadcnt=3, taskcnt=10\n", prog);
                     exit(EXIT_FAILURE);
        }
    }
    ...
    for (i=0; i<taskcnt; ++i) {
        ...
        threadpool_addtask(&pool, task_routine, arg);
    }
    sleep(10);
